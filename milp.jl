...
@constraint(m, ct_4[(i,j) for (i,j) in keys(distances) if i in I], to[i]+(times[i,j] + s[i])*x[i,j] - ls[1]*(1-x[i,j]) <= to[j])
## a concatener
@constraint(m, ct_4_bis[(1,j) in distances], to[1]+(times[1,j] + s[1])*x[1,j] - ls[1]*(1-x[1,j]) <= to[j])

@constraint(m, ct_5[(i,j) in distances if i in F_prime, j in 2:n+2], to[i]+times[i,j]*x[i,j] + g*(Q-y[i]) - (ls[1] + g*Q)*(1-x[i,j]) <= to[j])
@constraint(m, ct_6[j in 1:n+2], es[j] <= to[j])
@constraint(m, ct_7[j in 1:n+2], to[j] <= ls[j])
@constraint(m, ct_8[(i,j) in distances], u[j] <= u[i] - q[i]*x[i,j] + C(1-x[i,j]))
@constraint(m, ct_9, u[1] <= C)
@constraint(m, ct_10[(i,j) in distances if i in I], y[j] <= y[i] - r*distances[(i,j)]*x[i,j] + Q*(1-x[i,j]))
@constraint(m, ct_11[(i,j) in distances if i in F_prime or i==1], y[j] <= Q - r*distances[(i,j)]*x[i,j])

## OBJECTIF ##
@objective(m, Min, sum(distances[(i,j)]*x[i,j] for (i,j) in distances))
