...
include(string(folder_path, "data/data_extraction.jl"))
...
n_tot , r, g, Q, es, ls, I, F_prime,
    neighbours_in, neighbours_out, distances, times =
    extract_data(string(folder_path, "data/E_data_1.txt"))
...
@constraint(m, ct_1[i in I ; i!= 1 && i!=n+2],
    sum(x[i,j] for j in neighbours_out[i] if j!=1) == 1)
@constraint(m, ct_2[i in F_prime; i!= 1 && i!=n+2],
    sum(x[i,j] for j in neighbours_out[i] if j!=1) <= 1)
@constraint(m, ct_3[j in 2:n+1],
    sum(x[j,i] for i in neighbours_out[j] if i != 1) -
    sum(x[i,j] for i in neighbours_in[j] if i != n+2) == 0)
@constraint(m, ct_4[i in I, j in 2:n+2 ; (i,j) in keys(distances) && i != n+2],
    tau[i]+(times[(i,j)] + s[i])*x[i,j] - ls[1]*(1-x[i,j]) <= tau[j])
@constraint(m, ct_5[i in F_prime, j in 2:n+2;
    (i,j) in keys(distances) && i != 1 && i != n+2],
    tau[i]+times[(i,j)]*x[i,j] + g*(Q-y[i]) - (ls[1] + g*Q)*(1-x[i,j]) <= tau[j])
@constraint(m, ct_6[j in 1:n+2], es[j] <= tau[j])
@constraint(m, ct_7[j in 1:n+2], tau[j] <= ls[j])
@constraint(m, ct_8[i in 1:n+1, j in 2:n+2 ; (i,j) in keys(distances)],
    u[j] <= u[i] - q[i]*x[i,j] + C*(1-x[i,j]))
@constraint(m, ct_9, u[1] <= C)
@constraint(m, ct_10[i in I, j in 2:n+2;
    (i,j) in keys(distances) && i != 1 && i != n+2],
    y[j] <= y[i] - r*distances[(i,j)]*x[i,j] + Q*(1-x[i,j]))
@constraint(m, ct_11[i in F_prime, j in 2:n+2;
